...
| `parse_program(src)` | **Experimental** | Full-program ST parser (v4).  Covers PROGRAM / FUNCTION / FUNCTION_BLOCK declarations + all seven IEC §2.4.3 VAR_* blocks (VAR_INPUT / VAR_OUTPUT / VAR_IN_OUT / VAR / VAR_EXTERNAL / VAR_TEMP / VAR_GLOBAL) + IEC §2.4.1.1 AT clauses (`%IX0.0` / `%QW1` / `%MD2.3`) + IEC §2.3.3 TYPE blocks (STRUCT / ARRAY / ENUM / SUBRANGE / ALIAS) + IEC §2.7 CONFIGURATION / RESOURCE / TASK / VAR_ACCESS / VAR_CONFIG / PROGRAM-WITH bindings + body via the statement parser.  Out of scope (raises `StParseError`): METHOD / INTERFACE / EXTENDS / IMPLEMENTS / ABSTRACT, SFC text. |

...
| `parse_program(src)` | **Experimental** | Full-program ST parser (v6).  Covers PROGRAM / FUNCTION / FUNCTION_BLOCK declarations + all seven IEC §2.4.3 VAR_* blocks (VAR_INPUT / VAR_OUTPUT / VAR_IN_OUT / VAR / VAR_EXTERNAL / VAR_TEMP / VAR_GLOBAL) + IEC §2.4.1.1 AT clauses (`%IX0.0` / `%QW1` / `%MD2.3`) + IEC §2.3.3 TYPE blocks (STRUCT / ARRAY / ENUM / SUBRANGE / ALIAS) + IEC §2.7 CONFIGURATION / RESOURCE / TASK / VAR_ACCESS / VAR_CONFIG / PROGRAM-WITH bindings + IEC 3rd-edition OOP (METHOD inside FUNCTION_BLOCK with PUBLIC / PRIVATE / PROTECTED / INTERNAL access specs + ABSTRACT / OVERRIDE; INTERFACE blocks; EXTENDS / IMPLEMENTS / ABSTRACT modifiers on FUNCTION_BLOCK) + IEC §6.7 SFC text (INITIAL_STEP / STEP / END_STEP with `target(qual[, T#<ms>ms]);` action calls + TRANSITION FROM ... TO ... := ...; with `(a, b)` step lists for simultaneous div/conv + ACTION/END_ACTION inline bodies re-attached to step actions; AND/OR/NOT condition subset lowered to `ContactNO`/`ContactNC`/`ParallelGroup` ops) + body via the statement parser.  Out of scope (raises `StParseError`): CLASS / class-level OOP. |

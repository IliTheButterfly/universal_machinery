...
1. **PLCopen TC6 XML emitter + reader**.  ✅ *Validated against the
   ...
   Next:
     - Mixed LD+FBD bodies (rungs containing math / call /
       ...
       every Rung shape).
     - SFC `<macroStep>` (hierarchical sub-networks) and
       `<jumpStep>` (named jump targets); action blocks per
       IEC §2.6.4.4 and explicit selection / simultaneous
       divergence/convergence markers per IEC §2.6.3 both
       round-trip natively.
...
slices land; the current pass count is 1095.

...
1. **PLCopen TC6 XML emitter + reader**.  ✅ *Validated against the
   ...
   Next:
     - Mixed LD+FBD bodies (rungs containing math / call /
       ...
       every Rung shape).
     - SFC `<macroStep>` (hierarchical sub-networks); action
       blocks per IEC §2.6.4.4, explicit selection / simultaneous
       divergence/convergence markers per IEC §2.6.3, and
       `<jumpStep targetName=...>` for back-edge transitions all
       round-trip natively.
...
slices land; the current pass count is 1104.
